...
#' - \code{DT::dataTableOutput("list_of_tags")} prints the names of the time series \code{tags} chosen by the user. It is from this table that elements for plotting are selected. Clicking on a series in the plot area highlights it in this table. \cr
...
#' - A \code{selectInput} (multiple) box with \code{value_types} which should be plotted, which are populated from the \code{value_type} column of \code{time_series_data} in the server function \code{\link{time_series_plotting}}. This is accompanied by a \code{checkBox} highlight option to highlight aggregated series in plots, and grey out the individual components.
#' \cr \cr
...
time_series_plotting_ui <- function(id, module_header_text){
  ...
  tagList(
    H1(module_header_text),
    actionButton(ns("validate_and_proceed"), "Validate and proceed"),
    
    # data tables area
    fluidRow(
      DT::dataTableOutput(ns("list_of_tags"))
    ),
    fluidRow(
    ...
    fluidRow(textOutput(ns("clicked_series")))
    )
